...
### 集合函数
`array`, `append`, `map`, `orderedMap`, `stack`, `queue`, `seq`, `keys`, `values`

### 类型转换函数
`int`, `float`, `bool`, `string`, `byte`, `uint`, `char`, `bytes`, `chars`
- 静态方法：`int.parse()`, `float.parse()`, `string.parse()`, `bool.parse()`

### 对象引用函数
`ref()`, `deref()`, `setref()` - 对象引用和解引用支持

### Seq 类型（自增长序列）
`seq()` - 创建自增长序列，支持自动扩展索引访问
- 方法：`Append()`, `Pop()`, `Clear()`, `Resize()`, `Fill()`, `Range()`, `Reverse()`, `Join()`, `Includes()`, `IndexOf()`, `Get()`, `Set()`
...
- 返回 httpResponse 对象，包含字段：`statusCode`, `body`, `headers`
