...
func (c *Compiler) Compile(node parser.Node) error {
	...
	case *parser.WhileStatement:
		...
		for i := 0; i < len(c.currentInstructions()); i++ {
	...
	case *parser.ForStatement:
		...
		// Continue target is before update statement
	...
	case *parser.VarStatement, *parser.LetStatement, *parser.ConstStatement:
	...
	case *parser.ContinueStatement:
		...
		c.emit(OpJmp, loopCtx.continueTarget)
		return nil
...
func (c *Compiler) changeOperand(pos int, newOperand int, operandIndex ...int) {
	...
	if op == OpJmp || op == OpJmpIfTrue || op == OpJmpIfFalse || op == OpTry {
		// 2-byte operand
		if offset+1 >= len(c.currentInstructions()) {
			return
		}
		c.currentInstructions()[offset] = byte(newOperand >> 8)
		c.currentInstructions()[offset+1] = byte(newOperand & 0xFF)
	} else {
		// 1-byte operand
		if offset >= len(c.currentInstructions()) {
			return
		}
		c.currentInstructions()[offset] = byte(newOperand)
	}
}
